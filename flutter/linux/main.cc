...
#define RUSTDESK_LIB_PATH "libzdesk.so"
...
bool flutter_rustdesk_core_main() {
   void* libzdesk = dlopen(RUSTDESK_LIB_PATH, RTLD_LAZY);
   if (!libzdesk) {
      fprintf(stderr,"Failed to load \"libzdesk.so\"\n");
      char* error;
   ...
   auto core_main = (RustDeskCoreMain) dlsym(libzdesk,"rustdesk_core_main");
   char* error;
   if ((error = dlerror()) != nullptr) {
