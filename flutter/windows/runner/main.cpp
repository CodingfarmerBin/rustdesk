...
typedef char** (*FUNC_ZDESK_CORE_MAIN)(int*);
typedef void (*FUNC_ZDESK_FREE_ARGS)( char**, int);
typedef int (*FUNC_ZDESK_GET_APP_NAME)(wchar_t*, int);
...
{
  HINSTANCE hInstance = LoadLibraryA("libzdesk.dll");
  if (!hInstance)
  {
    std::cout << "Failed to load libzdesk.dll." << std::endl;
    return EXIT_FAILURE;
  }
  FUNC_ZDESK_CORE_MAIN zdesk_core_main =
      (FUNC_ZDESK_CORE_MAIN)GetProcAddress(hInstance, "zdesk_core_main_args");
  if (!zdesk_core_main)
  {
    std::cout << "Failed to get zdesk_core_main." << std::endl;
    return EXIT_FAILURE;
  }
  FUNC_ZDESK_FREE_ARGS free_c_args =
      (FUNC_ZDESK_FREE_ARGS)GetProcAddress(hInstance, "free_c_args");
  if (!free_c_args)
  {
  ...
  int args_len = 0;
  char** c_args = zdesk_core_main(&args_len);
  if (!c_args)
  {
    std::string args_str = "";
  ...
  std::wstring app_name = L"ZDesk";
  FUNC_ZDESK_GET_APP_NAME get_zdesk_app_name = (FUNC_ZDESK_GET_APP_NAME)GetProcAddress(hInstance, "get_zdesk_app_name");
  if (get_zdesk_app_name) {
    wchar_t app_name_buffer[512] = {0};
    if (get_zdesk_app_name(app_name_buffer, 512) == 0) {
